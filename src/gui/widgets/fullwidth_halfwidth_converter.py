...
class FullwidthHalfwidthConverterWindow:
    def __init__(self, parent, main_window):
        self.window = tk.Toplevel(parent)
        self.window.title("全角符号转半角符号工具")
    ...
    def _create_widgets(self):
        ...
        info_label = tk.Label(
            main_frame,
            text="将Excel文件中的全角字符（数字、字母、符号等）转换为半角字符\n支持拖拽文件到此窗口进行批量转换",
    ...
    def convert_files(self):
        ...
        # 如果没有设置输出路径，确认是否要覆盖原文件
        if self.output_path is None:
            result = messagebox.askyesno(
                "确认操作", 
                f"即将转换 {len(files)} 个文件，原文件将被覆盖。\n\n是否继续？",
                icon='warning'
            )
            if not result:
                return
