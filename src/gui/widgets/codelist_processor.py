...
class CodelistProcessorWindow:
    """Codelist处理窗口"""
    def __init__(self, parent, main_window):
        self.window = tk.Toplevel(parent)
        self.window.title("Codelist处理工具")
    ...
    def _create_widgets(self):
        ...
        upload_codelist_btn = tk.Button(
            file_frame,
            text="上传Codelist",
            command=self.select_codelist_file,
            width=15,
            height=1,
            font=fonts["body"],
            relief='flat',
        ...
        upload_codelist_btn.pack(side=tk.LEFT, padx=5)
        self.theme.style_button(upload_codelist_btn, variant="warning")
    ...
    def select_codelist_file(self):
        ...
        if file_path:
            try:
                # 调用处理器的select_codelist_file方法处理Codelist文件
                self.processor.select_codelist_file(file_path)
                self.status_label.config(text=f"已加载Codelist: {os.path.basename(file_path)}")
            except Exception as e:
                messagebox.showerror("错误", f"加载Codelist时发生错误: {str(e)}")
    ...
    def process_files(self):
        ...
        # 检查是否已加载Codelist
        if not hasattr(self.processor, 'codelist_file') or self.processor.codelist_file is None:
            messagebox.showwarning("警告", "请先上传Codelist文件！")
            return
