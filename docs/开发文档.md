...
- 数据模糊化 DM 专属设置新增两组替换启用开关（默认勾选），支持按规则独立停用替换。
- DM 专属设置布局改为“字段 + 替换值 + 启用开关”同行展示，并补充对齐与视觉分层规则（提示蓝/警示红）。
- 全量文档统一同步至 `v1.8.2`。
...
TOOL_PAGE_REGISTRY = [
    # ... existing pages
    ("new_tool", "src.gui.widgets.new_tool_page", "NewToolPage", SOME_ICON, "新工具"),
]
```
...
`packaging/pyinstaller/tools_box_onedir.spec` 为推荐链路（onedir）：
- 隐藏导入模块
- 数据文件收集
- 二进制文件过滤
- 保留与旧 spec 同级别兼容配置

`tools_box.spec` 继续保留，用于单文件兼容构建。
...
- 压缩和优化设置
- 平台特定的配置
- onedir + 安装包减少启动时自解压开销
- 主窗口按需懒加载工具页，降低首屏初始化成本
